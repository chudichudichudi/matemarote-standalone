...
gflags.DEFINE_string("locale_language","en","locale language",short_name='l')
gflags.DEFINE_string("game_folder","","path to game folder (where the manifest and 'game' folder are situated).",
    short_name='g')
gflags.DEFINE_integer("port",8000,"the port in which in the server listen.",short_name='p')
gflags.RegisterValidator('game_folder',is_valid_game_folder,message='game_folder must exist and be valid.\n\n')
...
def browser():
    try:
        ...
        d = threading.Thread(target=open_browser, args=(server,))
...
def sujeto():
    global boton_empezar
    boton_empezar.pack_forget()
    global log_file
    global window
    log_file = askstring(u'Metacognición', u'¿Nombre del sujeto?', parent=window)
    if log_file and len(log_file) > 0 and not os.path.isfile(log_file):
        Button(window,text='Terminar', command=close).pack(padx=50, pady=50)
        t = threading.Thread(target=browser)
        t.start()
    else:
        showwarning("Error", "Por favor escriba un nombre que no haya usado")

boton_empezar = Button(window,text='Empezar', command=sujeto, padx=50, pady=50)
boton_empezar.pack()
window.mainloop()
